...
for L = [TimeStamps' ; TrialID ; tsrecord_id]
    ...
    ROWS = length(L{1});
    ts = table('Size', [ROWS 9], 'VariableTypes', {'double', 'double', 'string', 'string', 'string', 'double' 'string', 'string', 'string'},'VariableNames',{'TrialNum', 'Frame', 'Timestamp', 'Epoch', 'Event', 'StimOnsetTime', 'Side', 'Choice', 'Score'});
    ts{:,3} = L{1};
    ...
    diffTimes(diffTimes<0) = 0; % Filter out any negative values, when the clock flips over
