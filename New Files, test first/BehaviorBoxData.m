   classdef BehaviorBoxData < handle
    ...
    properties
        ...
        start_tic; % tic handle for fast relative timing during live experiments
        date = [];
    ...
    methods
        ...
        function GetStartTime(this)
            % Fast timing: use tic/toc for relative timestamps (minutes)
        ...
        function addStimEvent(this, ~) %Get the timestamp of when the stimulus appeared
            % elapsed time in minutes since starting first trial (fast)
            if isempty(this.start_tic)
                this.start_tic = tic;
            end
            this.current_data_struct.TimeStamp(end+1) = toc(this.start_tic)/60;
        end
...
% Better method: match each frame timestamp to the stimulus timestamp vector
    ...
    methods(Static)
        ...
        % ============================
        % REVISED: Criterion analytics
        % ============================
        function T = TrialsToCriterion80(this, Tbl, options)
            % Compute trials-to-criterion (>=80% performance) per Level.
            %
            % Tbl: table with at least fields Score, Level. Optional fields:
            %   - Include (logical)
            %   - CodedChoice / WhatDecision (for filtering)
            %
            % options.Window (default 20): moving window size
            % options.Criterion (default 0.8)
            % options.IgnoreTimeout (default true): drops Score==2
            % options.Method: 'sliding' (default) or 'bayes'
            % options.BayesPrior: [a0 b0] default [1 1]
            % options.BayesProb: posterior P(p>criterion) threshold default 0.95
            % options.MinTrials: minimum trials before declaring criterion default Window
            arguments
                this
                Tbl table
            ...
            for iL = 1:numel(Levels)
                L = Levels(iL);
                w = Tbl.Level == L;
                y = double(Tbl.Score(w) == 1); % success vector
                N(iL) = numel(y);
    ...
    methods (Static)
        function idx = trialsToCriterionSliding(y, criterion, window, minTrials)
            % Fast sliding-window criterion crossing:
            % returns first trial index where mean of last window >= criterion.
            n = numel(y);
            if n < max(window, minTrials) || window <= 1
                idx = NaN;
                return
            end
            % Causal moving average via FIR filter
        ...
        function idx = trialsToCriterionBayes(y, criterion, a0, b0, probThr, minTrials)
            % Beta-Binomial sequential criterion:
            % idx = first trial where P(p>criterion | data) >= probThr.
            n = numel(y);
            if n < minTrials
                idx = NaN;
                return
            ...
            y = y(:);
            cs = cumsum(y);               % successes
            k = (1:n)';                   % trials
            a = a0 + cs;
            b = b0 + (k - cs);
            p = 1 - betacdf(criterion, a, b); % posterior prob p>criterion
