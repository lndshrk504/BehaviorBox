...
- Must support three modes:
  ...
  - full file logging
  - write-on-failure
- Production default must be in-memory only so routine training runs do not emit serial debug files by default.
- Full file logging must be explicitly gated by a debug flag or equivalent runtime setting.
- Write-on-failure mode must keep enough in-memory history to flush a useful debug artifact only when the session fails or a top-level error path is reached.
- When file output is enabled, logs must be written into a `Debug/` folder as structured log output
- Debug log filenames must include the date and time so separate runs do not overwrite one another
- In-memory history may still exist in all modes, but timestamped `Debug/` files are required only when full file logging is enabled or when write-on-failure flushes them after an error
...
- Write-on-failure mode must flush at least the recent in-memory serial history plus a small header containing:
  - `SchemaVersion`
  - `GeneratedAt`
  - `Workflow`
  - `FailureIdentifier`
  ...
  - `SessionSaveFolder`
  - `LogMode`
- The write-on-failure artifact format must be fixed to two files with the same stem:
  - one metadata file in JSON format
  - one CSV file containing the serial event rows
- The required filename stem should be:
  - `serial_failure_<class>_<yyyy_MM_dd-HH_mm_ss>`
- The required output files should therefore be:
  - `serial_failure_<class>_<yyyy_MM_dd-HH_mm_ss>.json`
  - `serial_failure_<class>_<yyyy_MM_dd-HH_mm_ss>.csv`
- The JSON metadata file must include at least:
  - `SchemaVersion`
  - `GeneratedAt`
  - `Workflow`
  - `FailureIdentifier`
  ...
  - `EventRowCount`
  - `ClassName`
- JSON field typing must be fixed:
  - `SchemaVersion`, `Workflow`, `FailureIdentifier`, `FailureMessage`, `SessionSaveFolder`, `LogMode`, and `ClassName` as strings
  - `MaxHistoryRows`, `MaxHistoryMinutes`, and `EventRowCount` as numeric scalars
  - empty unknown text values should be serialized as empty strings, not omitted fields
- `FailureIdentifier` sourcing must be deterministic:
  - first preference: the caught MATLAB `MException.identifier`
  - second preference: a normalized app-level identifier generated by the top-level catch path
  - final fallback: `BehaviorBox:UnknownFailure`
- `FailureIdentifier` should use MATLAB-style colon-delimited naming when synthesized, for example `BehaviorBox:TopLevelCatch` or `BehaviorBox:SerialFlushFailure`.
- `FailureMessage` sourcing must also be deterministic:
  - first preference: the caught MATLAB `MException.message`
  - second preference: a normalized app-level summary string from the catch path
  - final fallback: `Unknown failure`
- `FailureMessage` must be a single-line string in the JSON metadata.
- If the original exception message contains embedded newlines, those newlines must be normalized to spaces in the JSON metadata while the richer stack or detail text may remain elsewhere.
- The JSON metadata should additionally include:
  - `FailureSource`
  - `TopStackFrame`
- `FailureSource` must indicate which source path populated the identifier and message, using a fixed vocabulary such as:
  - `MException`
  - `AppCatch`
  - `Fallback`
- `TopStackFrame` must be the literal top stack frame from the captured failure context, serialized as a single `file:line` string when available, otherwise an empty string.
- `GeneratedAt` in JSON must use an ISO 8601 wall-clock timestamp with timezone offset, for example `2026-04-07T14:23:51-04:00`.
- The CSV file must use the same column contract defined for normal serial log rows.
- CSV column typing must also be fixed by convention:
  - `timestamp`, `direction`, `port`, `commandName`, `rawValue`, `eventType`, `status`, `matchKey`, and `details` as text columns
  - empty values should be written as empty fields, not sentinel strings such as `null` or `NaN`, unless the raw serial content itself is literally that text
- CSV `timestamp` values must use the same ISO 8601 timestamp format with timezone offset as the JSON metadata.
- The CSV header row must be present exactly once and must use the canonical column order:
  - `timestamp,direction,port,commandName,rawValue,eventType,status,matchKey,details`
- If both serial classes flush on the same failure, each class must write its own JSON and CSV pair using its own `<class>` stem component.
- If a filename collision occurs within the same second, append the same additive suffix convention used elsewhere, such as `_001`, to both paired files.
- The metadata JSON is the canonical source for failure context; the CSV is the canonical source for per-event history.
...
- The serial logging mode must be controlled by one explicit runtime flag contract rather than scattered booleans.
- The flag contract must include:
  - setting name `Debug_SerialLogMode`
  - storage in the normalized settings path produced by `readSettingsFromApp()`
  - inclusion in the app-settings manifest
  - optional inclusion in `SettingsSnapshot`
...
- `Debug_SerialLogMode` must use a small fixed vocabulary:
  ...
  - `file`
  - `failure`
- `memory` must mean in-memory only and must be the production default.
- `file` must mean continuous timestamped file logging into `Debug/`.
...
- The resolved flag value should be recorded in:
  ...
  - any serial logging smoke test that verifies mode behavior
- The in-memory retention policy must also be fixed and shared by both serial classes.
- The retention contract must include:
  - `MaxHistoryRows`
  - `MaxHistoryMinutes`
  - drop-oldest behavior when limits are exceeded
- The production default should retain at least the most recent 500 serial events or 10 minutes of history, whichever is smaller.
- `failure` mode must guarantee that enough history is retained to capture the immediate lead-up to a top-level session error.
- `file` mode may still keep the same in-memory ring buffer for parity, but the on-disk CSV remains the source of full-session history.
- The active retention values should be recorded in write-on-failure metadata and should be inspectable from debug smoke tests.
