...
%====================================================================
    ...
    methods
        ...
        function Ax = plotLvByDayOneAxis(this, options)
            arguments
                this
            ...
            for SUB = this.Sub
                ...
                %[daynums, Dates] = findgroups(this.AnalyzedData.DayMM{this.sc}.Ds');
                title = SUB{:}+" All Time Performance";
                f = figure("Name",title, "Visible", "off");
                ...
                for L = 1:maxPassedL
                    LO = L-1;
                    Ddat = this.AnalyzedData.DayMM{this.sc}(this.AnalyzedData.DayMM{this.sc}.Ls==L,:);
                    try
                        y = Ldat{L}(:,1)';
                        std = Ldat{L}(:,2)';
                        x = Ldat{L}(:,3)';
...
%                                 "HorizontalAlignment","center");
                ...
                Ax.Title.String = title;
                if options.save && numel(this.Sub)>1
                    %Save the figure
                    this.SaveManyFigures(f,title+".pdf")
                end
                time = seconds(datetime("now") - t1);
        ...
        function SaveManyFigures(this, fig, filename, format)
            arguments
                this
                fig
                filename string
        ...
        function [FigProps] = getFigProps(~, fig)
            chosen_figure=fig;
            figure_property = struct; %Reset export Settings:
            FigProps = struct;
            figure_property.units = 'inches';
            figure_property.format = 'pdf';
            figure_property.Preview= 'none';
            figure_property.Width= '32'; % Figure width on canvas
