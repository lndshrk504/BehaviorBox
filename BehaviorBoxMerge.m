        ...
        function WaitForInput(this)
            ...
            switch 1
                ...
                case this.Box.ardunioReadDigital==1 %Nose and Lick are Digital
                    ...
                    InpDelay = this.Setting_Struct.Input_Delay;
                    event = 0;
                    while ~get(this.stop_handle, 'Value') %While the stop button has not been pressed
                        ...
                        while this.a.readDigitalPin(this.Box.Middle) == this.Box.readHigh
                            ...
                            if this.a.readDigitalPin(this.Box.Middle) ~= this.Box.readHigh
                                set(this.message_handle,'String','Waiting for center choice...');
                                t1 = datetime("now");
                                break
                            else
                                event = 1; %Left Choice
        ...
        function Flash(Stim, Lines, whatdecision)
            ...
            function CorrectFlash
                for StimRep = 1:Reps
                    ...
                    pause(1/Freq/10)
                    [Lines(:).Color] = deal(Stim.LineColor); drawnow
                    pause(1/Freq/10)
                    [Lines(:).Color] = deal(Stim.FlashColor); drawnow
                    if StimRep < Reps
                        pause(1/Freq/10)
                    end
                end
